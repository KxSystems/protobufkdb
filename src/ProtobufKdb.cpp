...
K SerializeArray(K message_type, K msg_in, K use_field_names)
{
  ...
  if (use_field_names->t != -KB)
    return krr((S)"use_field_names not -KB");
...
K ParseArray(K message_type, K char_array, K use_field_names)
{
  ...
  if (char_array->t != KG && char_array->t != KC)
    return krr((S)"char_array not KG|KC");
  if (use_field_names->t != -KB)
    return krr((S)"use_field_names not -KB");
...
K SetStringKdbType(K kdb_type)
{
  if (kdb_type->t != -KH)
    return krr((S)"kdb_type not -KH");

  switch (kdb_type->h) {
  case -KS:
  case KG:
  case KC:
    kx::protobufkdb::KdbTypes::Instance()->SetStringKdbType(kdb_type->h);
    break;
  default:
    return krr((S)"kdb_type value not -11|4|10");
  }

  return (K)0;
}

K GetStringKdbType(K unused)
{
  return kh(kx::protobufkdb::KdbTypes::Instance()->GetStringKdbType());
}

K SetStringMapKeyKdbType(K kdb_type)
{
  if (kdb_type->t != -KH)
    return krr((S)"kdb_type not -KH");
  ...
  case KG:
  case KC:
    kx::protobufkdb::KdbTypes::Instance()->SetStringMapKeyKdbType(kdb_type->h);
    break;
  default:
    return krr((S)"kdb_type value not -11|4|10");
  }
...
K GetStringMapKeyKdbType(K unused)
{
  return kh(kx::protobufkdb::KdbTypes::Instance()->GetStringMapKeyKdbType());
}

K SetBytesKdbType(K kdb_type)
{
  if (kdb_type->t != -KH)
    return krr((S)"kdb_type not -KH");
  ...
  case KG:
  case KC:
    kx::protobufkdb::KdbTypes::Instance()->SetBytesKdbType(kdb_type->h);
    break;
  default:
    return krr((S)"kdb_type value not -11|4|10");
  }
...
K GetBytesKdbType(K unused)
{
  return kh(kx::protobufkdb::KdbTypes::Instance()->GetBytesKdbType());
}
